...
/**
 * 随机生成中文姓名，性别，Email，手机号，住址,金额，数字
 *
 * @author xmf
 */
public class RandomUtil {
    public static char[] provinceAbbr = { // 省份简称 4+22+5+3
            '京', '津', '沪', '渝',
    ...
    private static String citys[] = {"北京,广东,山东,江苏,河南,上海,河北,浙江,香港,山西,陕西,湖南,重庆,福建,天津,云南,四川,广西,安徽,海南,江西,湖北,山西,辽宁,内蒙古,郑州,开封,洛阳,平顶山,安阳,鹤壁,新乡,焦作,濮阳,许昌,漯河,三门峡,南阳,商丘,信阳,周口,驻马店,新郑市,新密市,登封市,荥阳市,巩义市 ,偃师市 ,舞钢市,汝州市 ,林州市 ,卫辉市,辉县市 ,沁阳市,孟州市 ,禹州市,长葛市 ,义马市,灵宝市 ,永城市  ,项城市 ,邓州市  ,济源市,石景山,房山,通州,顺义,昌平,大兴,怀柔,平谷,延庆,密云,西城,海淀,朝阳,丰台,门头沟"};
     ...
     * @return 定长的字符串
     */
    public static String getStringBumber(int len) {
        Random rand = new Random();
        for (int i = INTARRAY.length; i > 1; i--) {
            int index = rand.nextInt(i);
            int tmp = INTARRAY[index];
     ...
     * @return 定长的字符串
     */
    public static String getStringBumber(int[] array, int len) {
        Random rand = new Random();
     ...
     * @return 范围内的数字
     */
    public static int getDesAreaRandomNumber(int min, int max) {
        Random random = new Random();
    ...
    public static String getRandomID() {
        ...
        // 随机生成出生年月 7-14
        String birth = randomBirth(20, 50, "yyyyMMdd");
        // 随机生成顺序号 15-17(随机性别)
     ...
     * @return
     */
    public static String randomBirth(int minAge, int maxAge, String patten) {
        SimpleDateFormat dft = new SimpleDateFormat(patten);// 设置日期格式
        Calendar date = Calendar.getInstance();
        date.setTime(new Date());// 设置当前日期
